SELECT BATCH_ID, INPUT_SEQ, ORDER_NO, BOX_ID, BOX_TYPE
     ...
     , COLOR_CD
  FROM (
	     SELECT BATCH_ID, MIN(INPUT_SEQ) AS INPUT_SEQ, ORDER_NO, BOX_ID, BOX_TYPE_CD AS BOX_TYPE
	          , COUNT(1) AS SKU_QTY
	          , SUM(PICK_QTY) AS PLAN_QTY 
	          , NVL(SUM(PICKED_QTY),0) AS RESULT_QTY
	          , MAX(COLOR_CD) AS COLOR_CD
	      FROM (
	            SELECT BATCH_ID, ORDER_NO, MIN(INPUT_SEQ) AS INPUT_SEQ, BOX_ID, BOX_TYPE_CD
	                 , COM_CD, SKU_CD
	             ...
	             WHERE DOMAIN_ID = :domainId
	               ...
	               AND ORDER_TYPE = 'MT'
	             GROUP BY BATCH_ID, ORDER_NO, BOX_ID, BOX_TYPE_CD, COM_CD, SKU_CD
	           )
	     GROUP BY BATCH_ID, ORDER_NO, BOX_ID, BOX_TYPE_CD  
       )
