...
public class CompanyController extends AbstractRestService {
	...
	/**
	 * TODO 재사용 가능하게 MpsServiceUtil로 이동
	...
	private Page<?> searchByPagination(String siteOnlyVal, Integer page, Integer limit, String select, String sort, Filter[] filters) {
		...
		if(ValueUtil.isEqualIgnoreCase("true", siteOnlyVal)) {
			sql.add("  dc.domain_id as belongs_domain_id")
			   .add("from")
			   .add("  tb_company c inner join (select * from tb_domain_company where domain_id = :domainId) dc on c.com_cd = dc.com_cd")
			   .add("where 1=1");
			
		} else if(ValueUtil.isEqualIgnoreCase("all", siteOnlyVal)) {
			sql.add("  dc.domain_id as belongs_domain_id")
			   .add("from")
			   .add("  tb_company c left outer join (select * from tb_domain_company where domain_id = :domainId) dc on c.com_cd = dc.com_cd")
			   .add("where 1=1");
			...
			sql.add("  null as belongs_domain_id")
			   ...
			   .add("  tb_company c")
			   .add("where")
			   .add("  c.com_cd not in (select com_cd from tb_domain_company where domain_id = :domainId)");			
		}
		...
		Page<?> result = this.queryManager.selectPageBySql(sql.toString(), params, Company.class, page, limit);
		return result;
	}

	@RequestMapping(value = "/domain_companies", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ApiDesc(description = "Search Domain Company List")
	public Page<Code> domainCompanies() {
		StringJoiner sql = new StringJoiner(SysConstants.LINE_SEPARATOR);
		sql.add("select")
		   .add("  c.id, c.com_cd as name, c.com_nm as description")
		   .add("from")
		   .add("  tb_company c inner join tb_domain_company dc on c.com_cd = dc.com_cd")
		   .add("where")
	...
	public List<Company> checkImport(@RequestBody List<Company> list) {
		for (Company item : list) {
			this.checkForImport(Company.class, item);
		}
	...
	public Map<String, Object> receiveFromWcs() {
		...
		Object count = this.receiverFactory.getCompanyReceiver(domainId).receiveData(domainId);
